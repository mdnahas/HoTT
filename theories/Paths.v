...
(** Shorthand for the identity path. *)
...
Definition inverse {A : Type} {x y : A} (p : x = y) 
  : y = x
:= 
  match p with idpath => idpath end.
...
(** Naturality of [ap]. *)
Definition concat_Ap {A B : Type} {f g : A -> B} (p : forall x, f x = g x) 
    {x y : A} (q : x = y)
...
Definition ap10 {A B} {f g : A -> B} (h : f = g) (x : A) 
  : f x = g x
:= 
  apD10 h x.

Definition ap10_1 {A B} {f : A -> B} (x : A) 
  : ap10 (idpath f) x = 1
:= 
  1.
...
Definition whiskerL {A : Type} {x y z : A} (p : x = y) {q r : y = z} (h : q = r) 
  : p @ q = p @ r
:=
  1 @@ h.
...
Definition concat_whisker {A} {x y z : A} (p p' : x = y) (q q' : y = z) 
    (a : p = p') (b : q = q')
...
Definition eckmann_hilton {A : Type} {x:A} (p q : 1 = 1 :> (x = x)) 
  : p @ q = q @ p 
:=
...
Definition ap02 {A B : Type} (f:A->B) {x y:A} {p q:x=y} (r:p=q) 
  : ap f p = ap f q
:= 
  match r with idpath => 1 end.
